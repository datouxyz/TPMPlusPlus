...
*
*=========================================================================*/
#ifndef sitkTriangleThresholdImageFilter_h
#define sitkTriangleThresholdImageFilter_h
...
This filter creates a binary thresholded image that separates an image into foreground and background components. The filter computes the threshold using the TriangleThresholdCalculator and applies that threshold to the input image using the BinaryThresholdImageFilter .
...
\sa itk::simple::TriangleThreshold for the procedural interface
\sa itk::TriangleThresholdImageFilter for the Doxygen on the original ITK class.
     */
    class SITKBasicFilters_EXPORT TriangleThresholdImageFilter : public ImageFilter {
    public:
      using Self = TriangleThresholdImageFilter;

      /** Destructor */
      virtual ~TriangleThresholdImageFilter();
...
\
     ...
     double GetThreshold() const { return this->m_Threshold; };
      ...
      /** Name of this class */
      std::string GetName() const { return std::string ("TriangleThresholdImageFilter"); }
     ...
     * \brief Threshold an image using the Triangle Threshold.
     *
     * This function directly calls the execute method of TriangleThresholdImageFilter
     * in order to support a procedural API
     *
     * \sa itk::simple::TriangleThresholdImageFilter for the object oriented interface
     * @{
     */

     SITKBasicFilters_EXPORT Image TriangleThreshold ( const Image & image, const Image & maskImage, uint8_t insideValue = 1u, uint8_t outsideValue = 0u, uint32_t numberOfHistogramBins = 256u, bool maskOutput = true, uint8_t maskValue = 255u );
     SITKBasicFilters_EXPORT Image TriangleThreshold ( const Image & image, uint8_t insideValue = 1u, uint8_t outsideValue = 0u, uint32_t numberOfHistogramBins = 256u, bool maskOutput = true, uint8_t maskValue = 255u );
     /** @} */
  }
}
#endif
