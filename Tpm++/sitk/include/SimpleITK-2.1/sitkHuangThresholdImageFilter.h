...
*
*=========================================================================*/
#ifndef sitkHuangThresholdImageFilter_h
#define sitkHuangThresholdImageFilter_h
...
This filter creates a binary thresholded image that separates an image into foreground and background components. The filter computes the threshold using the HuangThresholdCalculator and applies that threshold to the input image using the BinaryThresholdImageFilter .
...
\sa itk::simple::HuangThreshold for the procedural interface
\sa itk::HuangThresholdImageFilter for the Doxygen on the original ITK class.
     */
    class SITKBasicFilters_EXPORT HuangThresholdImageFilter : public ImageFilter {
    public:
      using Self = HuangThresholdImageFilter;

      /** Destructor */
      virtual ~HuangThresholdImageFilter();
...
\
     ...
     double GetThreshold() const { return this->m_Threshold; };
      ...
      /** Name of this class */
      std::string GetName() const { return std::string ("HuangThresholdImageFilter"); }
     ...
     * \brief Threshold an image using the Huang Threshold.
     *
     * This function directly calls the execute method of HuangThresholdImageFilter
     * in order to support a procedural API
     *
     * \sa itk::simple::HuangThresholdImageFilter for the object oriented interface
     * @{
     */

     SITKBasicFilters_EXPORT Image HuangThreshold ( const Image & image, const Image & maskImage, uint8_t insideValue = 1u, uint8_t outsideValue = 0u, uint32_t numberOfHistogramBins = 128u, bool maskOutput = true, uint8_t maskValue = 255u );
     SITKBasicFilters_EXPORT Image HuangThreshold ( const Image & image, uint8_t insideValue = 1u, uint8_t outsideValue = 0u, uint32_t numberOfHistogramBins = 128u, bool maskOutput = true, uint8_t maskValue = 255u );
     /** @} */
  }
}
#endif
