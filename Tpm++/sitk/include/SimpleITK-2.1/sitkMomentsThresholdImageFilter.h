...
*
*=========================================================================*/
#ifndef sitkMomentsThresholdImageFilter_h
#define sitkMomentsThresholdImageFilter_h
...
This filter creates a binary thresholded image that separates an image into foreground and background components. The filter computes the threshold using the MomentsThresholdCalculator and applies that threshold to the input image using the BinaryThresholdImageFilter .
...
\sa itk::simple::MomentsThreshold for the procedural interface
\sa itk::MomentsThresholdImageFilter for the Doxygen on the original ITK class.
     */
    class SITKBasicFilters_EXPORT MomentsThresholdImageFilter : public ImageFilter {
    public:
      using Self = MomentsThresholdImageFilter;

      /** Destructor */
      virtual ~MomentsThresholdImageFilter();
...
\
     ...
     double GetThreshold() const { return this->m_Threshold; };
      ...
      /** Name of this class */
      std::string GetName() const { return std::string ("MomentsThresholdImageFilter"); }
     ...
     * \brief Threshold an image using the Moments Threshold.
     *
     * This function directly calls the execute method of MomentsThresholdImageFilter
     * in order to support a procedural API
     *
     * \sa itk::simple::MomentsThresholdImageFilter for the object oriented interface
     * @{
     */

     SITKBasicFilters_EXPORT Image MomentsThreshold ( const Image & image, const Image & maskImage, uint8_t insideValue = 1u, uint8_t outsideValue = 0u, uint32_t numberOfHistogramBins = 256u, bool maskOutput = true, uint8_t maskValue = 255u );
     SITKBasicFilters_EXPORT Image MomentsThreshold ( const Image & image, uint8_t insideValue = 1u, uint8_t outsideValue = 0u, uint32_t numberOfHistogramBins = 256u, bool maskOutput = true, uint8_t maskValue = 255u );
     /** @} */
  }
}
#endif
