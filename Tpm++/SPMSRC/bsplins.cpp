...
#pragma omp parallel for schedule(dynamic , 1)
    for(j=0; j<n; j++)
    {
        if (((cond&1) | (x0[j]>=1-TINY && x0[j]<=m0+TINY)) &&
			...
			/*f[j] = dsample3(c, m0, m1, m2, x0[j], x1[j], x2[j], d,
				&df0[j], &df1[j], &df2[j], bnd);*/
        else
            f[j] = NaN;
    }
	...
	OUT VECTOR_TYPE&f, VECTOR_TYPE &d1s, VECTOR_TYPE &d2s, VECTOR_TYPE &d3s)
{
	...
	for (int k = 0; k < 3; k++)
	{
		d[k] = floor(deg[k] + 0.5);
		if (d[k] < 0 || d[k]>7)
			ErrOutput("BSplinesD", "Bad spline degree.");
	}

	int cond = 0;
	for (int k = 0; k < 3; k++) bnd[k] = mirror;
	for (int k = 0; k < 3; k++)
	{
		if (deg[k + 3])
		{
	...
	f =	  VECTOR_TYPE(ncoords);
	d1s = VECTOR_TYPE(ncoords);
	d2s = VECTOR_TYPE(ncoords);
	d3s = VECTOR_TYPE(ncoords);

	dfun(c, m0, m1, m2, ncoords, x1.data(), x2.data(), x3.data(), d, cond, bnd, f.data(), d1s.data(), d2s.data(), d3s.data());

	return 0;
}

//dfun(double c[], int m0, int m1, int m2,
//	int n, double x0[], double x1[], double x2[], int d[],
//	int cond, int(*bnd[])(int, int),
//	double f[], double df0[], double df1[], double df2[])
...
VECTOR_TYPE BSplines(double *c, DimVec dim,VECTOR_TYPE &x0, VECTOR_TYPE &x1, VECTOR_TYPE &x2,DoubleVec &deg)
{
	...
	for (int k = 0; k < 3; k++)
	{
		d[k] = floor(deg[k] + 0.5);
		if (d[k] < 0 || d[k]>7)
			ErrOutput("BSplines", "Bad spline degree.");
	}
	...
	int ncoords = x0.size();
	VECTOR_TYPE fout(ncoords);
		
	fun(c, m0, m1, m2, ncoords, x0.data(), x1.data(), x2.data(), d, cond, bnd, fout.data());
